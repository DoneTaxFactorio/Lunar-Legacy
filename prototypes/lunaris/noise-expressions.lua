data:extend{
  {
    ...
    name = "lunaris_enemy_base_intensity",
    -- biter placement stops increasing in "intensity" after 75 chunks (2400 tiles)
    expression = "clamp(distance, 0, 2400) / 325"
  },
  {
    type = "noise-expression",
    name = "lunaris_enemy_base_radius",
    expression = "sqrt(var('control:lunaris_enemy-base:size')) * (15 + 4 * lunaris_enemy_base_intensity)"
  },
  {
    type = "noise-expression",
    name = "lunaris_enemy_base_frequency",
    -- bases_per_km2 = 10 + 3 * lunaris_enemy_base_intensity
    expression = "(0.00001 + 0.000003 * lunaris_enemy_base_intensity) * var('control:lunaris_enemy-base:frequency')"
  },
  {
    type = "noise-expression",
    name = "lunaris_enemy_base_probability",
    expression = "spot_noise{x = x,\z
                             y = y,\z
                             density_expression = spot_quantity_expression * max(0, lunaris_enemy_base_frequency),\z
    ...
    local_expressions =
    {
      spot_radius_expression = "max(0, lunaris_enemy_base_radius)",
  ...
  {
    type = "noise-function",
    name = "lunaris_enemy_autoplace_base",
    parameters = {"distance_factor", "seed"},
    expression = "random_penalty{x = x + seed,\z
                                 y = y,\z
                                 source = min(lunaris_enemy_base_probability * max(0, 1 + 0.002 * distance_factor * (-312 * distance_factor - starting_area_radius + distance)),\z
  ...
  },
  {
    ...
    parameters = {"max_l_elevation", "influence"},
    expression = "if(max_l_elevation >= l_elevation, influence * min(max_l_elevation - l_elevation, 1), -inf)"
  },
  {
    type = "noise-expression",
    name = "rock_noise",
